...
?>
<form action="register-encryption.php" method="POST" enctype="multipart/form-data">
...
if (strcasecmp($_SERVER['REQUEST_METHOD'], 'get') === 0) {
    ...
    if (count($errors) > 0) {
        ...
        // encrypt users name before storing it.
        $key = require __DIR__ . DIRECTORY_SEPARATOR . 'key.php';
        $values['naam'] = \Defuse\Crypto\Crypto::encrypt($_POST['naam'], $key);

        $values['password'] = password_hash($_POST['password'], PASSWORD_DEFAULT);
        file_put_contents($dataFilename, json_encode($values));
        ?>Dankjewel, <?=htmlentities($_POST['naam']);?> (voorheen <?=htmlentities(\Defuse\Crypto\Crypto::decrypt($storedValues['naam'], $key));?>) voor jouw registratie!<br><?php
        if ($uploadSuccess) {
